    ...
    if( $id > 0){
        ...
        if ($productos != null){
            foreach ($productos as $clave => $cantidad){
                $sql = $con->prepare("SELECT ID, Nombre, Precio FROM Servicios WHERE id=? AND URLI=1");
                $sql->execute([$clave]);
                $row_prod = $sql->fetch(PDO::FETCH_ASSOC);
        
                $sql_insert = $con->prepare("INSERT INTO detalle_compra (ID_compra, ID_servicio, Nombre, Cantidad, Precio) VALUES (?,?,?,?,?)");
